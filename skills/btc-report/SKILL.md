...
2. Output the `ai_analysis.free_text` field content exactly as-is. Do not reformat, restructure, translate, or add anything.
...
- `mcp__btc__get_btc_report` — returns JSON string with: `coin`, `ts`, `price_at_diag`, `direction` (bull/bear), `ai_analysis`
